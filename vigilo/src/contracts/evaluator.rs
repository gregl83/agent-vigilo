...
pub(crate) struct EvaluatorFinding {
    /// Evaluator-reported dimension for audit and diagnostics.
    ///
    /// Run profiles own the aggregation bucket through evaluator bindings.
...
/// Host-side normalized finding used before profile binding policy is applied.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub(crate) struct NormalizedEvaluatorResult {
    ...
    /// Evaluator-reported source dimension.
    pub(crate) dimension: EvaluationDimension,
...
/// Source-dimension grouping helper.
///
