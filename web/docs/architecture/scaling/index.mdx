...
- Control storage claims one open `run_id + run_shard` dispatch cursor with
...
- Control storage claims the run without waiting behind active dispatch
...
The normal move takes an exclusive transaction-scoped write-admission lock on
the source shard, revalidates its route fence, and marks the placement `moving`.
Worker claims, dispatch writes, and routed cancellation cleanup take shared
admission and revalidate the same alias, status, and `route_version`, so a stale
source route cannot write after movement begins. The move then checks for active
leased or running work, copies and verifies the shard, and compare-and-swaps the
route to the target alias with status `active`.
...
When active work exists, the route stays `moving` while the admitted work
drains; re-run the move afterward to resume it. `--force` remains accepted for
...
are eligible. Completion, failure, claimed cancellation, and drain deferral
require the claim's opaque fencing token. A shard with active work returns its
item to `pending` while its route remains `moving`; a later apply resumes it at
the expected next route version. Each item still uses the single-shard move
