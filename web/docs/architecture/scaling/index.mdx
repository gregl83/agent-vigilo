...
- Routed run creation, dispatch, worker processing, lease recovery, outbox
  publication, shard summaries, finalization, results, export, route
  inspection, and explicit shard move.
- CI coverage for the black-box multi-database route path: run creation to
  `primary` and `shard_001`, route inspection, control-owned cursors, and shard
  move.
