...
| Shard placement | Desired mapping from one used run shard to a database alias, lifecycle, route version, and write epoch. |
| Database router | Process-local resolver that turns stored placement metadata into a PostgreSQL pool. |
| Database circuit breaker | Process-local coordinator admission that temporarily skips one unavailable execution database alias without changing durable routing. |
...
| Write epoch | Ownership generation incremented only when write ownership moves or is restored. |
| Local shard admission | Execution-database row that accepts or rejects a write epoch in the same local transaction as the write. |
...
healthy alias from initializing. Coordinator runtime operations also pass
through a process-local per-alias circuit breaker. Open circuits defer durable
work for the unavailable alias while other aliases continue. Breaker state is
neither shared between coordinators nor persisted, and explicit administrative
commands bypass it.
...
- The worker resolves the hinted database pool locally and validates the
  execution database's admission row in the chunk-claim transaction. A valid
...
form of the shard advisory lock and validate the local admission state and
`write_epoch` in their execution-database transaction. The lock is cooperative:
every protected writer must explicitly acquire it. PostgreSQL does not infer
which rows belong to the shard.

`route_version` is the control-plane CAS token for lifecycle changes.
