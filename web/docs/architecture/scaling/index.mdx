...
flowchart LR
    ...
    admission["Local shard admission<br/>state + write_epoch + mover fence"]
...
- The **control database** owns evaluator registry rows, global run metadata,
  placement catalog rows, recoverable
  <GlossaryTerm term="run-creation-plan">run creation plans</GlossaryTerm>, dispatch cursors,
  coordinator leases, and global run lifecycle updates.
- An **execution database** owns high-volume run work: chunks,
  <GlossaryTerm term="run-snapshot">run snapshots</GlossaryTerm>, executions,
  attempts, aggregates, evaluator results,
  <GlossaryTerm term="run-shard-summary">run shard summaries</GlossaryTerm>, and
...
placement pools are resolved. A `provisioning` placement is registered but
cannot receive or serve ownership until activation verifies its target. An
`active` placement accepts new shard ownership. A `draining` placement rejects
new ownership but remains serviceable for shards it already owns, including
dispatch, workers, recovery, reads, outbox publication, and movement away from
the placement. A `disabled` placement is not serviceable.
...
- Each <GlossaryTerm term="route-hint">route hint</GlossaryTerm> in a worker
  message carries `run_id`, `run_shard`, `chunk_id`, `database_alias`, and a
  <GlossaryTerm term="write-epoch">`write_epoch`</GlossaryTerm>.
...
- On the terminal shard transition, each summary adds one bounded, mergeable entry per configured scorecard gate. Entries contain counts, score sums, and extrema; nonterminal refreshes do not scan scorecard inputs and summaries never contain execution rows.
...
- The coordinator verifies every shard scorecard has the same policy hash and gate set, merges exact counters, evaluates run gates, and atomically stores `run_scorecards`, `runs.gate_status`, and the completion event.
...
Add a shard-capable placement by migrating its target, deploying the secret env
var to every Vigilo process, registering it, and then activating it:

```bash
export VIGILO_SHARD_001_DATABASE_URL=postgres://...
DATABASE_URL="$VIGILO_SHARD_001_DATABASE_URL" vigilo setup --skip-evaluators
vigilo database register shard_001 --database-url-env VIGILO_SHARD_001_DATABASE_URL
vigilo database activate shard_001
```

Registration creates a non-routable `provisioning` catalog row. Activation is
bounded by `--database-operation-timeout-seconds` (default `30`) and verifies
that the target is reachable, writable, empty of local shard ownership, on the
same successful migration set as the control database, and physically distinct
from every active or draining placement. A failed check leaves the placement in
`provisioning`; fix the target and retry the idempotent activation command.
Activation validates the secret available to that command process, so deploy
the same environment variable to coordinators and workers before activating.
...
```mermaid
stateDiagram-v2
    state "Provisioning<br/>registered, not routable" as provisioning
    ...
    [*] --> provisioning: database register
    provisioning --> active: database activate<br/>readiness verified
...
| --- | --- | --- | --- |
| `provisioning` | Rejected | Rejected | Blocked |
| `active` | Allowed | Allowed | Served |
| `draining` | Rejected | Rejected | Served |
| `disabled` | Rejected | Rejected | Blocked |

Activation performs target I/O before taking the placement lifecycle lock, then
rechecks the registered role, environment reference, and absence of routing
references while atomically changing `provisioning` to `active`. This keeps slow
network checks outside the control transaction without exposing an unchecked
target. Concurrent activation attempts are idempotent.
...
then changes the route from `active` to `copying`. It installs the current
move ID, monotonic claim generation, and opaque claim token on the prepared
target before resetting or copying data. `copying` remains dispatchable while
the target is copied in durable row-and-byte-bounded pages.

```mermaid
stateDiagram-v2
    [*] --> active
    active --> copying: capture mutations, reserve target, install target fence
    copying --> copying: fenced backfill and replay
    ...
    moving --> active: final replay, activate target, clear target fence
...
writers.
The move claim generation is the target-writer fencing token. Every target
reset, copy, replay, or delete transaction holds shard admission and validates
the exact move ID, generation, and token before mutation. Installing a newer
generation takes exclusive target admission, waits for in-flight writes, and
prevents an expired coordinator from reinstalling or writing with older
authority.
...
storage. Capture state and dirty keys live on the source; mover authority lives
on the prepared target admission. Because the fenced target page commits before
its checkpoint, a lost acknowledgement replays at most one idempotent page
without growing control metadata per page. A rejected target write cannot
advance its control checkpoint. After backfill,
...
while admitted work settles. The exclusive source fence is held only for final
replay, an empty-journal check, and route activation.
...
- Provisioning, active, draining, and disabled database-placement lifecycle with atomic
