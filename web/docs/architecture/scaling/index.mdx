...
flowchart LR
    ...
    route["Shard placement<br/>run_id + run_shard<br/>alias + route_version"]
    router["DatabaseRouter"]
    target[("Execution database")]
    data["Chunks, executions, results,<br/>summaries, and outbox"]

    control --> route --> router --> target --> data
```
...
Placement status and role are checked in the control database before Vigilo
admits work. An `active` placement accepts new shard ownership. A `draining`
...
stateDiagram-v2
    ...
    [*] --> active: databases add
    active --> draining: databases drain
    draining --> disabled: databases disable
```

| Database status | New run shard | Placement set or move target | Existing owned shard |
| --- | --- | --- | --- |
...
vigilo shard databases drain shard_001
vigilo shard rebalance plan --from shard_001 --to shard_002 --max-items 100
vigilo shard rebalance apply <operation_id> --max-items 25
vigilo shard rebalance verify <operation_id> --max-items 25
vigilo shard databases disable shard_001
```

`databases drain` is idempotent and refuses the control-capable placement.
After it succeeds, existing routes continue normally while rebalance evacuates
them. `databases disable` requires the placement to be draining and have zero
...
`blocked`. It never prints the database URL.
...
persists the destination while changing the shard placement from `active` to
`draining`. New dispatch and worker claims stop, while cleanup for work admitted
before the transition can finish. Once leased chunks and running attempts reach
zero, the workflow changes the route to `moving`. At that point all source
writes, including cancellation cleanup, are frozen while the target is
reconciled and copied. PostgreSQL enforces that draining and moving routes have
a target distinct from their current owner, and that active routes have no
pending target.

```mermaid
stateDiagram-v2
    [*] --> active
    active --> draining: persist target and stop new work
    draining --> draining: admitted work remains
    draining --> moving: active work reaches zero
    moving --> active: reset, copy, verify, and activate target
```

Worker claims, dispatch writes, and routed cancellation cleanup take the shared
form of the same advisory lock and revalidate the alias, status, and
`route_version`. The lock is cooperative: every protected writer must
explicitly acquire it. PostgreSQL does not infer which rows belong to the
shard.
...
move checks for active leased or running work. After the shard is frozen, it
deletes rows for the exact `run_id + run_shard` from the target in reverse
foreign-key order, copies the source rows in bounded batches, verifies ordered
streaming BLAKE3 fingerprints and row counts, and compare-and-swaps the route to
the target alias with status `active`. Shared prerequisite rows are upserted and
are not deleted because other target shards may reference them.
...
work finishes. An interrupted `draining` or `moving` move can be retried only
with its persisted target alias. A `moving` retry resets the target shard rows
and repeats the bounded copy, making stale retained target data harmless when a
shard later moves back to a database. Repeating a move after fenced activation
returns a verified no-op, which makes a lost command response safe to retry.
