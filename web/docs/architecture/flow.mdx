...
1. **Run** is created
2. Dataset cases are grouped into shard-local **chunks**
3. The coordinator dispatches chunks through outbox event records and RabbitMQ
4. Workers process case **executions** through **attempts**
5. Evaluators produce append-only findings
6. Execution aggregates are computed
7. The **run** is finalized
8. Its completion event record is published as a broker message
...
| Term | Meaning here |
| --- | --- |
| Run | Durable evaluation of one dataset version with one run profile and agent target. |
| Chunk | Bounded range of dataset cases processed under one worker claim. |
| Execution | Durable evaluation of one dataset case against the agent target. |
| Attempt | One worker effort to complete an execution; retries create later attempts. |
| Evaluator finding | Append-only normalized observation emitted by an evaluator invocation. |
| Outbox event record | Durable database row inserted in the same transaction as a state change. |
| Broker message | RabbitMQ message published from an outbox event record. |

See the [Glossary](/docs/glossary) for routing, ownership, and scoring terms.
