...
5. Evaluators produce append-only <GlossaryTerm term="finding">findings</GlossaryTerm>
6. Required <GlossaryTerm term="evaluator-completeness">evaluator completeness</GlossaryTerm> is verified
7. Complete findings are aggregated
...
After all invocations settle, the worker compares their findings with the
resolved evaluator manifest. Every required binding must have a valid score and
must not have an error or skipped finding. Optional bindings are diagnostic and
...
- evaluator findings and the resulting aggregate are persisted together
