...
The runtime uses the following <GlossaryTerm term="cardinality">cardinalities</GlossaryTerm>:
...
database at a time. See [Scaling](./scaling/index.mdx) for routing and placement
details.
...
At creation, every evaluator reference is resolved to an immutable
<GlossaryTerm term="evaluator-execution-plan">evaluator execution plan</GlossaryTerm>.
The plan pins registry id, artifact hash, evaluator ABI, host adapter, runtime
version, and the complete scoring-policy hash. Each execution placement receives
the same plan and workers verify its hash before use.
...
- each pinned evaluator artifact and ABI identity is verified before its
  version-specific host adapter is selected
...
- atomically stores the final scorecard and `gate_status`
