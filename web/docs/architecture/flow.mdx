...
The worker sends run/attempt ids, the agent identity, the case input, and the full case envelope. The response is mapped into the evaluator `actual` envelope before evaluators run.
