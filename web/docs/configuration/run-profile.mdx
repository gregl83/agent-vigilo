...
The `dimension` on an
<GlossaryTerm term="evaluator-binding">evaluator binding</GlossaryTerm> is the
profile-owned aggregation bucket. Evaluator-emitted dimensions are preserved
as source metadata, but they do not choose the scoring bucket.
...
Every required evaluator binding must return at least one valid `passed` or
`failed` score and must not return an `error` or `skipped` finding. Incomplete
...
- no hard blocking <GlossaryTerm term="finding">finding</GlossaryTerm> fails or
  errors
...
- `fail_on_any_blocking_failure` (bool): when `true`, any blocking failed finding fails the execution even if the total `aggregate_score` is above the threshold. When `false`, blocking failures are still recorded in `blocking_failures`, but the overall score gate determines pass/fail. Errors and skipped findings from required evaluators always make evaluator completeness fail.
...
- `required` (bool, default `true`): requires this binding to return a valid score before aggregation. `false` declares a diagnostic-only evaluator and requires `blocking: false` with `weight: 0.0`; optional findings are persisted but never affect authoritative scores or gates.
...
- `passed` and `failed` findings with valid normalized scores are scoreable.
- A required evaluator that is missing, returns `error` or `skipped`, or returns no scoreable finding makes evaluator completeness fail. Vigilo records the reason in `execution_aggregates.summary.evaluator_completeness` and publishes no dimension or aggregate scores for that execution.
- Optional evaluators are diagnostic only. Their findings remain available for reporting but never affect dimension scores, blocking failures, or the aggregate score.
