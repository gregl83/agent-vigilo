...
- Produce one or more <GlossaryTerm term="finding">findings</GlossaryTerm> in
  `output.results`
- For a required profile binding, produce at least one `passed` or `failed`
  finding with a valid score. Empty, errored, skipped, or scoreless output makes
  the execution incomplete and withholds authoritative scores.
