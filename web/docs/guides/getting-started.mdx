...
```bash
docker compose -f infra/dev/docker-compose.yml up -d postgres rabbitmq
```

Set the local connection strings:

```bash
export DATABASE_URL='postgres://postgres:password@localhost:5432/agent_vigilo'
...
```powershell
$env:DATABASE_URL='postgres://postgres:password@localhost:5432/agent_vigilo'
