...
| Term | Meaning here |
| --- | --- |
| Run profile | Versioned agent invocation, evaluator binding, scoring, and gate configuration. |
| Dataset | Versioned collection of immutable evaluation cases. |
| Run | One durable evaluation of a dataset with a run profile. |
| Execution | Evaluation of one dataset case against the configured agent target. |
| Attempt | One worker effort to complete an execution; retries create later attempts. |
| Finding | One normalized observation emitted by an evaluator. |
| Gate status | The run's policy outcome, separate from its operational status. |

See the [Glossary](/docs/glossary) for canonical definitions used across the documentation.
